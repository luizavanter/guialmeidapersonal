<template>
  ...
  >
    ...
    <div v-if="!selectedFile && !uploading">
      ...
      <p class="text-smoke font-medium mb-1">{{ label || 'Arraste um arquivo ou clique para selecionar' }}</p>
      <p class="text-stone text-sm">Clique para navegar</p>
      <p v-if="maxSizeMb" class="text-stone/60 text-xs mt-2">Tamanho máximo: {{ maxSizeMb }} MB</p>
    </div>
